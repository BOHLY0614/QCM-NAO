...
class QCMApp(tk.Tk):
    def __init__(self):
        super().__init__()
        # ... (Tout le début de ton __init__ reste identique jusqu'à self.question_stats) ...
        try:
            myappid = 'QCM NAO' 
    ...
    # --- GESTION AUDIO (Tes méthodes existantes restent ici) ---
    def create_volume_adjusted_file(self, file_path, volume):
        # ... (Code inchangé) ...
        try:
            with wave.open(file_path, 'rb') as wav_in:
                if wav_in.getsampwidth() != 2: return file_path
    ...
    # --- SCREAMER / JUMPSCARE (NOUVEAU) ---
    # --- SCREAMER / JUMPSCARE (VERSION SYNCHRO SON) ---
    def trigger_jumpscare(self):
        """Déclenche le jumpscare, calcule la durée du son pour le crash"""
        scream_dir = os.path.join(self.base_dir, "Assets", "Screamers")
        
        if not os.path.exists(scream_dir):
            return False

        # 1. Lister les GIF disponibles
        gifs = [f for f in os.listdir(scream_dir) if f.lower().endswith('.gif')]
        if not gifs:
            return False
            
        chosen_gif = random.choice(gifs)
        base_name = os.path.splitext(chosen_gif)[0]
        
        # 2. Chercher le son associé (WAV)
        sound_path = os.path.join(scream_dir, f"{base_name}.wav")
        gif_path = os.path.join(scream_dir, chosen_gif)
        
        # 3. SAUVEGARDE D'URGENCE
        backend.save_stats(self.question_stats)
        
        # 4. Calcul de la durée du son (POUR LE CRASH)
        sound_duration_ms = 2000 # Valeur par défaut (2s) si échec lecture
        if os.path.exists(sound_path):
            try:
                with wave.open(sound_path, 'rb') as wav_file:
                    ...
                    sound_duration_ms = int(duration * 1000)
            except Exception as e:
                print(f"Erreur lecture durée wav: {e}")

        # 5. Préparer l'interface (Ecran noir total)
        self.stop_sound()
        for widget in self.winfo_children():
            widget.destroy()
            
        self.configure(bg="black", padx=0, pady=0)
        self.attributes('-fullscreen', True)
        
        black_frame = tk.Frame(self, bg="black")
        black_frame.pack(fill="both", expand=True)
        
        lbl_anim = tk.Label(black_frame, bg="black", borderwidth=0)
        lbl_anim.pack(expand=True)
        
        # 6. Lecture du son (ASYNC pour ne pas bloquer le GIF)
        if os.path.exists(sound_path) and os.name == 'nt':
            import winsound
        ...
        # 8. Animation (Boucle infinie jusqu'au crash)
        def animate(index):
            # On boucle le GIF : si on arrive à la fin, on repart à 0
            # Cela permet d'avoir une image qui bouge même si le son est plus long que le GIF
            current_frame = frames[index % len(frames)]
            lbl_anim.configure(image=current_frame)
            self.after(30, animate, index + 1)

        animate(0)

        # 9. PROGRAMMATION DU CRASH
        # On ferme l'appli exactement quand le son se termine
        self.after(sound_duration_ms, self.destroy)
        
        return True

    def apply_theme(self):
        # ... (Inchangé) ...
        super().update() # Trick pour éviter des bugs graphiques parfois
        theme = self.themes[self.theme_mode]
        # ... (Reste de la méthode apply_theme inchangé) ...
    ...
    # --- MENU PRINCIPAL (Inchangé jusqu'à start_quiz) ---
    def create_main_menu(self):
        # ... (Copier coller ta fonction create_main_menu telle quelle) ...
        ...
        if SHOW_DEBUG_BUTTON:
            debug_btn = tk.Button(
                button_frame, text="🛠️ TEST SCORE / DEBUG", command=self.debug_show_score,
                font=("Arial", 10, "bold"), bg="#f1c40f", fg="black"
            )
        ...
        for i, chapter_path in enumerate(self.chapter_files):
            ...
            btn = ttk.Button(
                row_frame, text=button_text, command=lambda i=i: self.start_quiz(i), style="Large.TButton"
            )
            btn.pack(side='left', fill='x', expand=True)
        ttk.Separator(button_frame, orient='horizontal').pack(fill='x', pady=20)
        mix_btn = ttk.Button(
            button_frame, text="Lancer le QCM sur la sélection", command=self.start_mixed_quiz_selected, style="Large.TButton"
        )
        mix_btn.pack(pady=5, fill='x')
        error_btn = tk.Button(
            button_frame, text="⚠️ Revoir mes erreurs", command=self.start_error_review,
            font=("Arial", 14, "bold"), bg="#e74c3c", fg="white", relief="flat", pady=10
        )
    ...
    def start_quiz(self, chapter_index):
        # ... (Inchangé) ...
        if hasattr(self, 'unbind_scroll'): self.unbind_scroll()
        self.stop_sound()
        self.main_menu_frame.pack_forget()
    ...
    def start_mixed_quiz_selected(self):
        ...
        if not selected_indices:
            messagebox.showwarning("Attention", "Veuillez sélectionner au moins un chapitre.")
            return
        selected_questions = []
    ...
    def start_error_review(self):
        ...
        if not error_questions:
            messagebox.showinfo("Félicitations !", "Aucune erreur enregistrée.")
            return
    ...
    def restart_quiz(self):
        ...
        if self.last_chapter_index == -1: self.mix_all_chapters()
        elif self.last_chapter_index == -2: self.start_error_review()
        else: self.start_quiz(self.last_chapter_index)
    ...
    def next_question(self):
        ...
        if self.easter_egg_enabled_var.get() and random.random() < SCREAMER_PROBABILITY:
            if self.trigger_jumpscare():
                return 
